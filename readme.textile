...
The instructions above are for setting up the config *above* webroot (your "public_html", "htdocs", etc directory). This is recommended for security. However, if you're unable to do this due to hosting restraints Alex Ball has written about the changes necessary to support this. You can read "his step-by-step here.":http://alexball.tv/blog/using-focus-labs-master-config-in-webroot/
